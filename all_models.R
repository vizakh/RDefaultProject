all_models <- function(data, predict_dates, seasonality, title) {
  data_vasicek <- VasicekModel(data, predict_dates, 
                               title = glue::glue("Модель Васічека ({title})"), 
                               legend_pos = c(0.2, 0.85))
  data_linear <- linear_trend_seasonality(data, predict_dates, seasonality, 
                                          title = glue::glue("Лінійна регресія з трендом та сезонністю ({title})"),
                                          legend_pos = c(0.2, 0.85))
  data_poly <- poly_trend_seasonality(data, predict_dates, seasonality, 
                         title = glue::glue("Поліноміальна регресія з трендом та сезонністю ({title})"), 
                         legend_pos = c(0.2, 0.85))
  data_gam <- gen_add_model(data, predict_dates, seasonality, 
